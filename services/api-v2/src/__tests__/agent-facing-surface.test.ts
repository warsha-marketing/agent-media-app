...
// make_subtitles, make_product_hero, make_reaction, make_hands_on) + create_character (the one V2 tool that survives) +
...
describe('curated agent surface (A7/A10)', () => {
  it('exactly make_ugc, make_podcast, make_subtitles, make_product_hero, make_reaction, make_hands_on are agentFacing', () => {
    const facing = Object.values(SKILLS)
      .filter((s) => s.agentFacing === true)
      .map((s) => s.slug)
      .sort();
    expect(facing).toEqual(['make_hands_on', 'make_podcast', 'make_product_hero', 'make_reaction', 'make_subtitles', 'make_ugc']);
  });
