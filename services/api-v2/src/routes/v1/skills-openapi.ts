...
export function skillRouteOpenApi(): { paths: Record<string, unknown>; schemas: Record<string, unknown> } {
  ...
  const quote = {
    post: {
      ...
      parameters: [SLUG],
      responses: {
        '200': { description: 'The quote: credits, available (after reservations), committed, sufficient; for a Preset render also music_bed (on, track_id, mood, reason off|no_tracks, detail), in_use_reference (null, or on a Preset with hands or a person when the draft has one — made free when drafting, never priced here: status made with used, use_original_product_photo, image_url, used_state, removed_parts — set use_original_product_photo: true to render the hands and person shots from the original photo; or status failed with used false and a message: the render uses the original photo) and, for a Preset with its own inputs, preset_inputs (what renders: the resolved modesty; make_hands_on: hand_gender, setting and source user|product_details for each; make_reaction: character_id, character_gender)' },
  ...
  const shotPlan = {
    post: {
      ...
      responses: {
        '200': {
          description:
            'skill, preset, draft_id, duration_ms, set (the Short’s Set { set_id } or null), fields[] (the editable fields in composition order: id, label, max_chars or choices, required, people_only), and shots[]: shot_id (stable, independent of position: the shot’s role in the Preset, e.g. reaction or product-closer, with -2 only when a longer Short repeats the role), number, kind, shows (product | hands | person), clip_seconds, on_screen_ms, starting_frame, model { id, name }, fallback { id, name } | null (the first fallback), fallbacks [{ id, name }] (every fallback, in the order tried), set_id, fields { framing, scene, blocking, environment_interaction, performance, action, energy (calm | natural | lively), camera_move, lens_feel, lighting } (editable), default_fields (the Preset’s, for "reset"), edited_fields[], edited, guardrails { image[], video[] } of { id, label, text, enforced_by prompt | request } (locked, per stage: image is the starting frame’s, empty without one), prompt_preview { image | null, video } (the final prompts with the images named in plain words)',
        },
        '400': skillError(sentences('`invalid_input`: the body fails the skill schema', ...refusalLines(400))),
