...
/**
 * The Product Interaction guardrail check (#25).
 *
 * A Product Interaction adds an action to every hands and person shot; it must
 * never contradict the Guardrails the render puts in those prompts: nobody
 * speaks (ADR 0001: the draft's voice is the only speech) and the Modesty
 * Default (arms covered or sleeved, a hijab kept on). The render words the
 * interaction as "every instruction above still holds", but a video model
 * given "takes off her hijab" or "talks to the camera" may still follow it, so
 * text that says so is refused when it is SAVED: the writer's own output (one
 * rewrite told why, then an error, like the Script check) and the user's edit
 * on re-voice (422 PRODUCT_INTERACTION_BREAKS_GUARDRAIL).
 *
    ...
    for (const [g, re] of [...EN, ...AR]) {
      ...
      if (m) {
        ...
        return {
          guardrail,
          matched,
          message: `The Product Interaction breaks a Guardrail: "${matched}" — ${WHY[guardrail]}. Describe only how the hands use the product.`,
        };
      }
    }
  }
