...
/**
 * The Modesty Default's prompt wording (#17) — server-side, like the shot
 * prompts it is appended to (the levels and rules are public, in
 * @agentmedia/schema modesty.ts).
 *
 * The shared render pipeline appends it to every shot whose kind shows a person
 * or hands, and to no other (presetShotPrompt). Pure data + pure functions: the
 * workflow sandbox imports this.
 */
