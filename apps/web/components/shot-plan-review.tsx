...
/**
 * Shot Plan review (#26) on /dashboard/product-hero: an optional, collapsed
 * "Review shots" panel before Confirm. One card per shot the render will make —
 * number, kind, on-screen length, model → fallback — with its scene text
 * (editable, "Reset to Preset") and its Guardrails as locked chips. The server
 * composes the plan and always adds the Guardrails itself; this panel only
 * reports the changed scene text (lib/shot-plan-flow.ts shotEditsOf), which the
...
export function ShotPlanReview({
  ...
  choice: RenderChoice;
  disabled: boolean;
  /** The changed scene text, by shot id, whenever a card is left or reset. */
  onEdits: (edits: Record<string, string>) => void;
}) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setPlan(parsed);
      setTexts(Object.fromEntries(parsed.shots.map((s) => [s.shotId, choice.shotEdits?.[s.shotId] ?? s.sceneText])));
    })();
    return () => {
      live = false;
    };
  ...
  const edited = plan ? Object.keys(shotEditsOf(plan, texts)).length : 0;
      ...
      {open ? (
        <div className="flex flex-col gap-3 px-5 pb-5">
          <p className="text-xs" style={muted}>
            ...
            shot; the locked lines are always added by the server. Editing never changes the price.
          </p>
          {error ? (
            ...
            plan.shots.map((shot) => {
              const value = texts[shot.shotId] ?? shot.sceneText;
              const changed = tidyScene(value) !== tidyScene(shot.defaultSceneText);
              const hint = changed ? sceneTextHint(value, plan.sceneTextMax) : null;
              return (
                <article key={shot.shotId} className="flex flex-col gap-2 rounded-xl p-4" style={inner} aria-label={`Shot ${shot.number}`}>
                  ...
                  <label className={label} style={muted} htmlFor={`scene-${shot.shotId}`}>
                    Scene{changed ? <span className="ml-2 normal-case tracking-normal" style={{ color: '#C9B8FF' }}>edited</span> : null}
                  </label>
                  <textarea
                    ...
                    rows={4}
                    onChange={(e) => setTexts((cur) => ({ ...cur, [shot.shotId]: e.target.value }))}
                    onBlur={() => commit(texts)}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      ...
                      onClick={() => {
                        const next = { ...texts, [shot.shotId]: shot.defaultSceneText };
                        setTexts(next);
                        commit(next);
                      }}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <span className={label} style={muted}>Guardrails · always added</span>
                    <ul className="flex flex-wrap gap-1.5">
                      {shot.guardrails.map((g) => (
